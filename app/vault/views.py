...
'''# GET, PUT, and DELETE individual passwords for a unique user
@csrf_exempt
def unique_password_one_user(request, user):
    query = request.GET.get('url')
    try:
        password = Password.objects.filter(user=user).filter(url=query)
    except Password.DoesNotExist:
        return HttpResponse(status=404)
